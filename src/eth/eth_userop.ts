...
import { IMSCAService } from "./msca_service.js";
import { ENTRYPOINT_ADDRESS } from "src/paymaster/constants.js";
import { paymasterAddress } from "src/paymaster/paymaster.factory.js";
...
export const create_init_code = async (userService: IMSCAService) => {
    ...
    userService.accountFactory.interface.encodeFunctionData("createAccount",[salt,[eoa]])
    .slice(2);
...
export const formatUserOp = async (ethService: IMSCAService, msca: string, initCode: string, target: string, callData: string, token: string) : Promise<any> => {
    ...
    let userOp: any =  {
        ...
        paymasterAndData: paymasterAddress("ARB_SEPOLIA"),
