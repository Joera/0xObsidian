...
export class UpdateByIdStatement extends StatementHistory {
    #orbis;
    #id;
    #newValue;
    #replaceValues;
    constructor(orbis, documentId) {
        super();
        this.#orbis = orbis;
        this.#id =
    ...
    id(documentId) {
        const id = typeof documentId === "string" ? documentId : documentId.toString();
        if (this.#id) {
    ...
    replace(newValue) {
        if (this.#replaceValues) {
            console.warn("[QueryBuilder:update] Replacing existing value settings set by .set().");
    ...
    set(partialValues) {
        if (this.#newValue) {
            console.warn("[QueryBuilder:update] Replacing existing value settings set by .replace().");
    ...
    async run() {
        ...
        let document, error;
        if (this.#newValue) {
            [document, error] = await catchError(() => this.#orbis.ceramic.updateDocument(this.#id, this.#newValue));
        }
        else {
            [document, error] = await catchError(() => this.#orbis.ceramic.updateDocumentBySetter(this.#id, async (document) => {
                return {
                    ...document.content,
                    ...this.#replaceValues,
                };
